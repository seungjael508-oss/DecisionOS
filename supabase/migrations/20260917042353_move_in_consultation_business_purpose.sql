-- Add an explicit business purpose; old clients retain their original behavior when omitted.
-- No historical consultation rows are rewritten. Deploy this RPC before the new form.
drop function public.create_move_in_consultation(uuid, uuid, uuid, text, text, text, timestamptz, public.occupancy_intent, public.funding_status, public.move_in_status, date, timestamptz, timestamptz, text);
...
revoke all on function public.create_move_in_consultation(
  ...
  date, timestamptz, timestamptz, text, text
) from public;
